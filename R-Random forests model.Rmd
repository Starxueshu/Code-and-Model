...
```{r}
library(xlsx)
star<- read.csv("test_set_with_predictions-Random forests model.csv")
...
```{r}
tiff(filename="Decision_curve_Random forest.tif", width=600*6, height=600*6, res=90*6)
par(mfrow=c(1,1), mar=c(4.2,4.2,3,.5), pty="m")
plot_decision_curve(modelt,curve.name=c('Random Forest'),xlim=c(0,0.8),ylim=c(0,0.6),
...
```{r}
library(tidyverse)
tiff(filename="zhifangtu-random forests.tif", width=800*6, height=400*6, res=90*6)
...
tiff(filename="Calibration_curve_Random Forest.tif", width=600*6, height=600*6, res=90*6)
...
tiff(filename="Calibration_curve-Random Forest.tif", width=600*6, height=600*6, res=90*6)
plot(cali,
     xlim=c(0,1),
     ylim=c(0,1),
...
tiff(filename="xiangshitu-Random Forest.tif", width=800*6, height=600*6, res=90*6)
par(mfrow = c(1,1), pty='s', mar=c(4,3,4,1),cex=1, font =1, col=1)
...
ggplot(data=star,aes(x = anxietyanddepression, y =lr_pred_proba)) +
  ...
  scale_fill_manual(values=c("green", "red"))+
    ggtitle(paste("Random Forest, Discrimination Slope=", 
