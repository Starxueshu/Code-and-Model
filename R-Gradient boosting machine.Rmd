...
```{r}
library(xlsx)
star<- read.csv("test_set_with_predictions-Gradient boosting machine model.csv")
...
```{r}
tiff(filename="Decision_curve_Gradient boosting machine.tif", width=600*6, height=600*6, res=90*6)
par(mfrow=c(1,1), mar=c(4.2,4.2,3,.5), pty="m")
plot_decision_curve(modelt,curve.name=c('Gradient Boosting Machine'),xlim=c(0,0.8),ylim=c(0,0.6),
...
```{r}
library(tidyverse)
tiff(filename="zhifangtu-gradient boosting machine.tif", width=800*6, height=400*6, res=90*6)
...
tiff(filename="Calibration_curve_Gradient Boosting Machine.tif", width=600*6, height=600*6, res=90*6)
...
title("Gradient Boosting Machine", adj=0.5)
dev.off()
```
...
tiff(filename="Calibration_curve-Gradient boosting machine.tif", width=600*6, height=600*6, res=90*6)
plot(cali,
     xlim=c(0,1),
     ylim=c(0,1),
...
legend(0.55,0.35,
       ...
       bty="n")
title(paste("Gradient Boosting Machine"))
dev.off()
```
...
tiff(filename="xiangshitu-Gradient Boosting Machine.tif", width=800*6, height=600*6, res=90*6)
par(mfrow = c(1,1), pty='s', mar=c(4,3,4,1),cex=1, font =1, col=1)
...
ggplot(data=star,aes(x = anxietyanddepression, y =lr_pred_proba)) +
  ...
  scale_fill_manual(values=c("green", "red"))+
    ggtitle(paste("Gradient Boosting Machine, Discrimination Slope=", 
